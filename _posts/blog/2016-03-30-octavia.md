...
- 将amphorae db中的状态设置为pending delete
- 设置amphora-health db中的busy字段为True
- 调用Nova接口删除amphorae虚拟机（按照Nova的设计，删除虚拟机，不会删除虚拟机的port）
- 删除amphora-health db中该amp的记录
- 将amphorae db中的状态设置为deleted
- 创建新的amphorae虚拟机并配置haproxy和/或keepalived
