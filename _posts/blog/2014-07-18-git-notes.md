...
当错误使用`git commit --amend`后（原本是想独立的commit）如何恢复？`git reset --soft @{1}`，同时可以用`git commit --reuse-message @{1}`或`git commit -C @{1}`拿到刚刚的commit message。[这里](http://stackoverflow.com/questions/38001038/how-to-undo-a-git-commit-amend)有详细的解释。
